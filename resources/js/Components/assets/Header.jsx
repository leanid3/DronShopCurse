...
export default function Header({auth}) {
  return (
    ...
    <header className=' flex fixed items-center justify-evenly bg-gradient-to-r from-lightblue to-50% w-full md:w-screen  to-darkblue from-30% p-4 z-10'>
      <Link className="text-lightgrey text-center md:text-left text-xl md:text-3xl font-bold">DronShop</Link>
      ...
      <div className='flex text-center md:text-left gap-3 *:text-lightgrey *:flex *:flex-col *:items-center '>
        ...
        <Link href="#"><FaPercent className='text-xl md:text-3xl'/> <p className='hidden lg:block'>Акция</p></Link>
          ...
          {auth.user
            ? 
            <Link  href={route('dashboard')}><HiOutlineUser  className='text-xl md:text-3xl'/> <p className='hidden lg:block'>Профиль</p></Link>
